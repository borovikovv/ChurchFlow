  ...
  return (
    <div className="group/cell relative" title={note ?? undefined}>
      ...
      <button
        ...
        aria-label={hasNote ? t('editCellNote') : t('addCellNote')}
      ...
      <FloatingPortal>
        {open ? (
          <div
            ...
            {...getFloatingProps({
              className:
                'z-50 grid max-h-[min(320px,calc(100vh-32px))] w-[min(310px,calc(100vw-48px))] gap-3 overflow-auto rounded-lg border border-[var(--line)] bg-[var(--surface)] p-3 text-left shadow-[0_12px_36px_rgba(31,35,40,0.18)]',
            })}
          >
            ...
            <div className="flex items-center gap-2">
              {hasNote ? (
                <Button className="truncate" type="button" variant="ghost" onClick={clearNote}>
                  {t('clear')}
                </Button>
              ) : null}
              <Button
              ...
              <Button className="truncate" type="button" onClick={saveNote}>
                {t('save')}
              </Button>
            </div>
          </div>
