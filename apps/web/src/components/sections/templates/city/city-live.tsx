...
export function CityLive({
  ...
  const storedSecondaryHref = readText(content, 'secondaryHref');
  ...
  return (
    <section className="px-5 py-14 lg:py-[104px]" id="live">
      <div className="mx-auto grid w-full max-w-[1240px] bg-[#0a0a0a] text-white lg:grid-cols-[7fr_5fr]">
        {/* Without a stream to open, the poster stays a picture: a play control that goes nowhere
            would promise a video the website cannot show. */}
        {watchHref ? (
          <a
          ...
          <div className={`block aspect-video ${CITY_COVER_CLASS}`}>
            <LivePoster badge={null} content={content} play={false} />
          </div>
        )}
